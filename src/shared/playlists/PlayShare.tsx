...
function PlayShare({ cover, link }: Props) {
  const [showModal, setShowModal] = useState(false);
  const [copied, setCopied] = useState(false);

  const linkOnClick = async (socialMedia: string) => {
      ...
      >
        <Box sx={style}>
          <div className="w-full h-1/2 bg-[#f3f3f3] rounded-2xl flex !justify-center items-center">
            <div className="relative w-5/6 aspect-video rounded-2xl shadow-2xl overflow-hidden">
              <Image fill alt="" className="h-full w-full object-cover" src={cover} />
            </div>
            <button
